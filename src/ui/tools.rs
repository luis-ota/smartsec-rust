...
fn render_header(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    let title = if detecting {
        format!("{} Analyzing project at: {} ...", spinner, app.url_input)
    } else {
...
fn render_tool_list(app: &mut AppState, frame: &mut Frame, area: Rect) {
    ...
    let items: Vec<ListItem> = app
        ...
        .enumerate()
        .map(|(i, t)| {
            let is_cursor = i == app.tool_cursor && app.mode == AppMode::Assisted;
    ...
    if app.mode == AppMode::Assisted && !app.tool_detecting {
